...
// same schedule:
//
//   step k:  acc += 197 (12 bits); on overflow a new eighth begins -> tick()
//            phase(acc >> 8)                     -> late misses
...
const INC = 197;           // tempo increment per step (see src/tempo.v)
const ACC = 4096;          // the 12-bit accumulator wraps here
...
class Run {
  constructor(name) {
    ...
    this.expect = [];      // [step, b0, b1, b2, b3, b4, b5, b6, b7] on change
    this.seeds = [];
    this.last = '';
  }
  ...
  state() {
    ...
    if (!this.playing) {
      ...
      return [this.attractFrame, 0, (8 << 3) | 0, 0, 0, 0, 0, 0];
    }
    const n = c.note;
    return [
  ...
  get t() { return this.chip.abs * PH + (this.acc >> 8); }
  ...
  play(n, player) {
    for (let i = 0; i < n; i++) {
      ...
      if (acc >= ACC) { acc &= ACC - 1; abs++; }
      const tNext = abs * PH + (acc >> 8);
